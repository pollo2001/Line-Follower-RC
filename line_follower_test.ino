...
#define NOMINALSPEED        80 // This is the base speed for both motors, can also be increased by using potentiometers'

#define OVERSHOOT           4 //this is the constant we use to conpensate for overshoot, subkject to change
...
void ReadPotentiometers() {
  ...
  kPRead = ReadPotentiometerHelper(P_pin, 0, 1023, 0, 80); // We want to read a potentiometer for P_pin with resolution from 0 to 1023 and potentiometer range from 0 to 100.
  ...
  kDRead = ReadPotentiometerHelper(D_pin, 0, 1023, 0, 60); // We want to read a potentiometer for D_pin with resolution from 0 to 1023 and potentiometer range from 0 to 100.

} // end ReadPotentiometers()

//want to return float
float ReadPotentiometerHelper(int pin, int min_resolution, int max_resolution, int min_potentiometer, int max_potentiometer) {
...
void RunMotors() {
  ...
  error_scale = (abs(error) > 1) ? abs(error)/OVERSHOOT : 0.0;
  
  M1SpeedtoMotor = min(NOMINALSPEED + SpRead*(1-error_scale) + M1P, 255); // limits speed to 255
  M2SpeedtoMotor = min(NOMINALSPEED + SpRead*(1-error_scale) + M2P, 255); // remember M1Sp & M2Sp is defined at beginning of code (default 60)
